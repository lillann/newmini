concrete ExtMiniGrammarLat of ExtMiniGrammar = open MiniResLat, Prelude in {
  ...
  lincat
    ...
    -- Ant    = Ante;
     ...
     VP  = {v : GVerb; compl : Temp => Agreement => Str};
  ...
  lin
    ...
    TPres = MiniResLat.TPres;
    TPast = MiniResLat.TPast;
    ASimul = Sim;
    AAnter = Anter;
...
--    Third (Gender => Number => Case => String)
  --  {s = table {g => "omnis"; n }-- {s = "every" ; n = Sg} ;
    ...
    UseV v' = {
       v = verb2gverb v';
       compl = table {t => table {Agr _ n p  => v'.s ! t !  n ! p}}};
...
--     s : VForm => Str ;
  ...
  --   } ;
    ...
    --  VPres n p
    --  Agr g n p => v.s ! (VPres n p)
     
    compl = table {Agr _ n p t => (v'.s ! (Vf n p t)) ++ v2.c ++ np.s ! Ack};
     } ;
      
  
    
    ComplVS vs s = let v' = verb2gverb {inf = vs.inf; s = vs.s} in {
       v       = v';
       compl   = table { t => table { Agr _ n p  => v'.s ! (Vf n p t) ++ vs.conj ++ s.s}}};
    ...
    PredVP np vp = 
      let 
        typ  = np.typ;
        subj = case typ of {NPPron => ""; _ => np.s ! Nom} ;
     --   agr  = np.a;
     --   verb =  agrV vp.v np.a ;
        compl : Temp -> Str = \t -> 
            case np.a of {
             Agr g n p =>  (vp.v).s ! (Vf n p t) }-- vp.compl ! t ! np.a}
                  
      in {
        s = \\t,b => subj ++  case b of {False => "non"; True => ""} ++ compl t
      } ;
 ...
 AdvVP vp adv =
   vp ** {compl = \\t,agr => vp.compl ! t ! agr ++ adv.s};
   
   -- table {agr => vp.compl ! agr ++ adv.s}} ;
