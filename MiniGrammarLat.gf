concrete MiniGrammarLat of MiniGrammar = open MiniResLat, Prelude in {
 ...
 Pol = {s : Str ; b : Bool} ;
  ...
  lin
    ...
    UseV v = {
       v = verb2gverb v;
       compl = table {Agr _ n p => v.s ! n ! p}};
       
    
         
    ComplV2 v2 np = {
      v =  verb2gverb v2 ;
      compl = \\_ => v2.c ++ np.s ! Ack;
      } ;
    ...
    PredVP np vp = 
      ...
      in {
        s = \\b => subj ++  case b of {False => "non"; True => ""} ++ verb ++ compl
      } ;
      
      UseAP ap = {
        v   = be_GVerb ;
     --   obj = table {Agr g n p => ap.s ! g ! n ! Nom};
        compl = table {Agr g n p => ap.s ! g ! n ! Nom}   -- Gender => Number => Case => Str
        } ;
        
    AdvVP vp adv =
      vp ** {compl = table {agr => vp.compl ! agr ++ adv.s}} ;
